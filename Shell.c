...
//Here we have the main loop. In it, we first read in user input, split it into an array of arguments, and then punt it off for execution.
//This loop continues until the exit command is called. This command returns status == 0, while any other returns status == 1.
void sh_loop() {
  char *line;
  char **args;
  int status;
...
//Here we handle the task of reading in the user's input. We initialize a buffer, then loop indefinitely until an end of line character
//is read. Then we return to the main loop with the filled buffer. If our initial buffer or any later buffer is full, then we reallocate
//a new one that is larger before checking for more input. The code could be shorter by using certain newer C commands, but you hide details.
char *sh_read_line() {
...
//If the launcher is called, then we were passed a command we don't have built in. We then fork to create a new process
//and use the execvp call to attempt to run the command. If it can't be run for any reason, we print an error message.
int sh_launch(char **args) {
	pid_t pid, wpid;
	int status;
